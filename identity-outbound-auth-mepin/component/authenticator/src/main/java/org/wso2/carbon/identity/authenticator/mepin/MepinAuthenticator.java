...
public class MepinAuthenticator extends AbstractApplicationAuthenticator
    ...
    protected void initiateAuthenticationRequest(HttpServletRequest request,
        ...
        try {
            ...
            if (StringUtils.isNotEmpty(authenticatedLocalUsername)) {
                ...
                if(StringUtils.isNotEmpty(mepinID) && Boolean.parseBoolean(
                        authenticatorProperties.get(MepinConstants.MEPIN_LINKED_ENABLED))){
                    isLinked=true;
                }
            }
        } catch (UserProfileException e) {
            throw new AuthenticationFailedException("Unable to retrieve the associated user.");
        }
        ...
        try {
            ...
            response.sendRedirect(response.encodeRedirectURL("mepinauthenticationendpoint/temp.jsp" + "?authenticators=" + getName()
        ...
        /*p.print("<form id='f' method='post' action='https://localhost:9443/mepinauthenticationendpoint/mepin.jsp'>" +
                "<input type='secret' id='authenticators' name='authenticators' value='" + getName()+ "'/>" +
                "<input type='secret' id='applicationId' name='applicationId' value='" + authenticatorProperties.get(MepinConstants.MEPIN_APPICATION_ID) + "'/>" +
                "<input type='secret' id='callbackUrl' name='callbackUrl' value='" + authenticatorProperties.get(MepinConstants.MEPIN_CALLBACK_URL)+ "'/>" +
                "<input type='secret' id='sessionDataKey' name='sessionDataKey' value='" + context.getContextIdentifier() + "'/>" +
                "<input type='secret' id='isSecondStep' name='isSecondStep' value='" + isSecondStep+ "'/>" +
                "<input type='secret' id='isLinked' name='isLinked' value='" + isLinked + "'/>" +
                "</form>");
        p.print("<script type='text/javascript'>document.getElementById('f').submit()");*/

        /*p.print("<html xmlns='http://www.w3.org/1999/xhtml' xml:lang='en'>" +
                "<body onload='document.forms[0].submit()'>" +
                "<jsp:useBean id='myBean' scope='request' class='example.myBean' />" +
                "<form action='<jsp:getProperty name='myBean' property='https://localhost:9443/mepinauthenticationendpoint/mepin.jsp'/>' method='post'>" +
                "<input type='hidden' name='isLinked' value='<jsp:getProperty name='myBean' property='"+isLinked+"'/>'/>");
        p.print("</form>" +
                "</body>" +
                "</html>");*/
    ...
    public List<Property> getConfigurationProperties() {
        ...
        Property linkedEnabled = new Property();
        linkedEnabled.setName(MepinConstants.MEPIN_LINKED_ENABLED);
        linkedEnabled.setDisplayName("linkedEnabled");
        linkedEnabled.setRequired(true);
        linkedEnabled.setDescription("Enter Linked or not");
        linkedEnabled.setDisplayOrder(10);
        configProperties.add(linkedEnabled);
